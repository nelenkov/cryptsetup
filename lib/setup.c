				 ...
				 struct crypt_params_verity *params)
{
    ...
    log_dbg("params->data_size: %llu", params->data_size);
	if (!params->data_size) {
	...
	void *params)
{
    ...
    log_dbg("crypt_format %s, %s", type, uuid);
	if (!type)
		return -EINVAL;
...
int crypt_resize(struct crypt_device *cd, const char *name, uint64_t new_size)
{
	...
	log_dbg("Resizing device %s to %llu sectors.", name, new_size);
	...
	if (new_size == dmd.size) {
		log_dbg("Device has already requested size %llu"
			" sectors.", dmd.size);
		r = 0;
	} else {
		dmd.size = new_size;
...
{
	log_dbg("Timeout set to %llu miliseconds.", timeout_sec);
...
{
	log_dbg("Iteration time set to %llu miliseconds.", iteration_time_ms);
